...
pub mod pallet {
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn counter_dispute(
            ...
            let consumer = commitment.consumer; // Copy before move, same pattern as merkle_root above
            commitment.status = CommitmentStatus::DisputeResolved;
            VectorCommitments::<T>::insert(commitment_id, commitment);
            ActiveCommitmentCount::<T>::mutate(|n| *n = n.saturating_sub(1));

            // Reputation consequence: the consumer's dispute was proven
            // baseless. Fire-and-forget, must not block this extrinsic.
        ...
        pub fn finalize_dispute(
            ...
            // Reputation consequence: fire-and-forget, must not block
            // this permissionless extrinsic (see ReputationHandler docs).
